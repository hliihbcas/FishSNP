...
if [[ $N -eq 2 ]];then
  ...
  zcat ${wkdir}/00_data/${PROJECT}/${SRR}_fastx.gz |grep  '^@ERR' |cut -d' ' -f1 |sort |uniq -c |awk  '{if($1==2){print $2 >> "'${wkdir}'/00_data/'${PROJECT}'/'${SRR}'_highQpairedreads.list"}else{print $2 >> "'${wkdir}'/00_data/'${PROJECT}'/'${SRR}'_highQsinglereads.list"}}'
